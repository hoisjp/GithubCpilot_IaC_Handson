...
4. 想定されるリスクと対策を 3 点
5. 上記 1〜4 の内容を `docs/architecture.md` に保存すること
...
1. **Copilot に初期版を生成させる** — Copilot Chat に以下のプロンプトを貼り付けて実行します:

    ````text
    architecture.md の内容から architecture.drawio を作成して。
    ````

    Copilot が draw.io (mxGraph) XML を生成します。生成された XML を `docs/architecture.drawio` (または `docs/architecture.drawio.svg`) として保存します。

2. **draw.io で開いて確認** — VS Code の Draw.io Integration 拡張機能でファイルを開き、構成要素と接続線が正しく生成されているか確認します。

3. **Azure アイコンに差し替える** — 左ペイン下部の **「More Shapes...」** → **「Networking」カテゴリの `Azure`** にチェック → Apply し、以下の対応表に従って各要素のアイコンを Azure ステンシルに変更します:
...
4. **線・ラベルを確認・修正** — Copilot が生成した接続線とラベルを確認し、必要に応じて修正します:
...
- `docs/architecture.md` — Copilot が生成した設計内容 (Mermaid 図・リソース役割表・リスク一覧)
- `docs/architecture.drawio` — draw.io で清書したアーキテクチャ図 (編集ソース / git 管理対象)
- `docs/architecture.drawio` からエクスポートした SVG / PNG — プレゼン・README 用資料
