...
## 6. 設計図を「清書」する — Copilot に drawio.svg を作らせる

Mermaid はレビュー・議論のスピードが速い一方、**Azure リソースの見た目が抽象的** で、ステークホルダーへの説明資料としては物足りないことがあります。

そこで、3 章で作成した `docs/architecture.md` の **Mermaid 図を Copilot Chat に貼り付けるだけ** で、Azure 公式アイコンを使った `docs/architecture.drawio.svg` を生成してもらいます。

### 6-1. 環境準備 (1 回だけ)

`.drawio.svg` を VS Code 上でプレビュー・編集できるように、以下の拡張機能を入れておきます。
...
`docs/architecture.drawio.svg` に保存してください。
...
3. 生成された `docs/architecture.drawio.svg` を VS Code で開き、**Draw.io Integration 拡張のプレビュー** で図が正しく表示されるか確認します。
...
| `docs/architecture.drawio.svg` | 6 章の手順で作成 | draw.io で清書した資料用アーキテクチャ図 |
...
Copilot が生成した `docs/architecture.drawio.svg` は、汎用的な四角や円で表現されていることがあります。より丁寧に資料として仕上げたい場合は、**Azure 公式アイコン** に手動で差し替えてみてください。

> ℹ️ この手順は任意です。次の Step 2 に進むには必須ではありません。

### ① Azure アイコンライブラリを有効化する

1. VS Code で `docs/architecture.drawio.svg` を開きます (Draw.io Integration が起動します)。
...
- 保存は `Ctrl + S` のみで OK。`.drawio.svg` なので、git にコミットすれば差分レビューもできます。

> 💡 **Tip:** アイコン差し替えが面倒なときは、②の表を Copilot Chat に貼って「この対応表に従って `docs/architecture.drawio.svg` のアイコンを差し替えて」と依頼すれば、Agent モードが自動で編集してくれます。
